...
class FilterNavBar extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      getFilters(this.state.parameters.city, this.state.parameters.movieId, this.state.parameters.movieTheaterId)
        .then(options => {
    ...
    setCity = (city) => {
      getFilters(city, this.state.parameters.movieId, 'All cinemas')
        .then(options => {
    ...
    setMovieTheater = (movieTheaterId) => {
      getFilters(this.state.parameters.city, this.state.parameters.movieId, movieTheaterId,)
        .then(options => {
          const date = this.getCurrentDate();
    ...
    setMovie = (movieId) => {
      getFilters(this.state.parameters.city, movieId, this.state.parameters.movieTheaterId,)
        .then(options => {
  ...
  render() {
    ...
    return (
      options && <div className="header__filter_bar">
        <FilterComponent options={options.cities} key='1' defaultValue={this.state.parameters.city} setMethod={this.setCity}/>
        <FilterComponent options={options.movieTheaters} key='2' defaultValue={this.state.parameters.movieTheaterId} setMethod={this.setMovieTheater} name='cinemas'/>
